...
exports.handler=async(event)=>{
  ...
  return singleFlight(cacheKey,async()=>{
   ...
   try{
    if(supabaseConfigured()){
      const cached=await getCache(cacheKey);if(cached){const picks=filterUpcomingPicks(cached.picks);return json(200,{...cached,picks,persistence:true,cached:true,coalesced:true,stalePicksRemoved:(cached.picks||[]).length-picks.length});}
